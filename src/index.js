...
function ValueCard() {
    ...
    if (cardRegex.test(inputVal)) { //validamos la exp. regular si se cumple

       
       
       var textoParrafo=document.createTextNode("La tarjeta  " + fourDigits + msgeUser);// se crea el texto
       ...
       var messagePrvCard= parrafo.appendChild(document.createTextNode("El proveedor de su tarjeta es: " + cardProvider)) //Agrega texto al parrafo creado
       ...
       messageDiv.insertBefore(messagePrvCard, mssageProvider); //inserta el nuevo parrafo creado con el texto despues del primer parrafo del div padre
        ...
        var messageVa= document.createTextNode("La tarjeta  " + fourDigits + " es invalida, recuerda usar solo números sin espacios")
...
function messageUser(validCard) { // esta función recibe un booleano como parametro y retorna un mensaje
    var message;
    (validCard) ? message = " es una tarjeta VALIDA" : message = " es una tarjeta INVALIDA";
